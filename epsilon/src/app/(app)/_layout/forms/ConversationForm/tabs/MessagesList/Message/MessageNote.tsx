...
function MessageNote({ index }: MessageNoteProps) {
    ...
    const [note, , noteHelpers] = useField(`messages[${ index }].epsilonNote`);
        ...
        <>
            <Stack direction="row" justifyContent="space-between" alignItems="center">
                <Tooltip title={ isOpen ? 'Hide note' : 'Add/View note' }>
                    <IconButton onClick={ toggleNote } sx={ { padding: '4px' } }>
                        { isOpen ? (
            ...
            { isOpen ? (
                <DenseInput
                    name={ `messages[${ index }].epsilonNote` }
                    label="Note"
                    fullWidth
                    multiline
                    minRows={ 2 }
